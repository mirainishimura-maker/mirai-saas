...
// para recorrer Notaluma sin cuenta y sin datos reales.
//
...
// Respaldo en memoria para cuando localStorage no está disponible: en modo
// incógnito, con la cuota llena o con el almacenamiento bloqueado, la muestra
// tiene que seguir funcionando dentro de la pestaña.
let enMemoria = null

function leer() {
  try {
    const crudo = window.localStorage.getItem(LLAVE)
    if (!crudo) return enMemoria || estadoSemilla()
    return { ...estadoSemilla(), ...JSON.parse(crudo) }
  } catch {
    return enMemoria || estadoSemilla()
  }
}
...
function escribir(estado) {
  ...
  try {
    ...
    // Sin almacenamiento persistente: la muestra sigue en memoria.
  }
  return estado
}
...
export async function cambiarEstadoCita(citaId, status, { tarifa, patient_id, dia }) {
  ...
  mutar((e) => {
    if (!e.citas.some((c) => c.id === citaId)) return e
