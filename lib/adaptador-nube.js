...
export async function cambiarEstadoCita(citaId, status, { tarifa, patient_id, dia }) {
  ...
  if (status === 'Completed') {
    const { error: errorCobro } = await sb.from('financial_transactions').upsert(
      {
        therapist_id: sesion.user.id,
        patient_id,
    ...
    reventar(errorCobro, 'registrando el cobro')
  } else {
    const { error: errorBorrado } = await sb
      .from('financial_transactions')
      .delete()
      .eq('appointment_id', citaId)
    reventar(errorBorrado, 'quitando el cobro')
  }
