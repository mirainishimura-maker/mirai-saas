...
function reventar(error, quehacia) {
  if (!error) return
  console.error(`Notaluma · ${quehacia}:`, error)
...
export async function guardarNota({
  ...
  const sb = supabase()
  // La fecha se resuelve acá también, y no solo en el store: quien devuelve
  // la nota al estado tiene que devolverla completa, sin campos sin definir.
  const fecha = session_date || claveDia(new Date())
